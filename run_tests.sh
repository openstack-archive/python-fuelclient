...
usage() {
    ...
    echo ""
    echo "  -6  --python-26             Run python-2.6 tests."
    echo "  -7  --python-27             Run python-2.7 tests."
...
process_options() {
    ...
    TEMP=$(getopt \
        -o 67hnc:f:r:t: \
        --long python-26,python-26,help,no-clone,fuel-commit:,fetch-repo:,fetch-refspec:,tests: \
        -n 'run_tests.sh' -- "$@")
    ...
    while true ; do
        case "$1" in
            -6|--python-26) python_26=1;            shift 1;;
            -7|--python-27) python_27=1;            shift 1;;
