...
function usage {
    echo "Usage: $0 [OPTIONS] [-- [TESTR OPTIONS] [TEST FILE]]"
    ...
    echo "  -r  --fetch-refspec         Refspec to fetch from the remore repo."
    echo "  -c  --fuel-commit           Checkout fuel-web to a specified commit"
...
function process_options {
    # Read the options
    TEMP=`getopt -o hnc:f:r: --long tests,help,no-clone,fuel-commit:,fetch-repo:,fetch-refspec: -n 'run_tests.sh' -- "$@"`
    eval set -- "$TEMP"
...
function run_cli_tests {
    ...
    NAILGUN_ROOT=$NAILGUN_ROOT tox -epy26 -- -vv $testropts --xunit-file $FUELCLIENT_XUNIT || return 1
    popd >> /dev/null
...
#
# Creates databases and loads default data.
function setup_db {
    local config=$1
    local defaults=$2
