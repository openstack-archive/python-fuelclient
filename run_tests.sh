...
usage() {
    ...
    e_info "  -i  --integration-tests     Run only integration tests."
    ...
    e_info "  -p  --pep8                  Runs only flake8 tests."
    e_info "  -P  --no-pep8               Do not run flake8 tests automatically."
    ...
    e_info "  -u  --unit-tests            Run only unit tests."
...
prepare_env() {
    ...
    run_server $server_log $config || \
        { e_error "Failed to start Nailgun"; return 1; }
}
...
run() {
    ...
    if [[ $do_pep8 -eq 1 ]]; then
        e_error "Starting PEP8 tests..."
        run_pep8 || pep8_ret=$?

        [[ $pep8_ret -ne 0 ]] && e_error "Failed tests: pep8"
    ...
    e_error "Starting python-fuelclient tests..."
    run_cli_tests $config || cli_ret=$?
    [[ $cli_ret -ne 0 ]] && e_error "Failed tests: cli_tests"

    run_cleanup $config

    if [[ $pep8_ret -ne 0 || $cli_ret -ne 0 ]]; then
        e_error "Testing python-fuelclient failed."
        exit 1
    else
        e_error "Testing python-fuelclient succeeded."
        exit 0
    fi
}
