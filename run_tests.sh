...
process_options() {
    # Read the options
    TEMP=`getopt -o hnc:f:r:t: --long tests,help,no-clone,fuel-commit:,fetch-repo:,fetch-refspec:,tests: -n 'run_tests.sh' -- "$@"`
    eval set -- "$TEMP"
...
run_cli_tests() {
    ...
    NAILGUN_ROOT=$NAILGUN_ROOT tox -epy26 -- -vv $testropts $certain_tests --xunit-file $FUELCLIENT_XUNIT || return 1
    popd > /dev/null
