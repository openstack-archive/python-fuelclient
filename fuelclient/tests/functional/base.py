...
class BaseTestCase(oslo_base.BaseTestCase):
    ...
    def wait_task_ready(self, task_id, timeout=60, period=3):
        """Wait for changing task's status to 'ready'.

        :param task_id: Task ID
        :type task_id: str or int
        :param timeout: Max time of waiting in seconds
        :type timeout: int
        :param period: Period for getting task info in seconds
        :type period: int
        """
        cmd_show_task = "task show -f json {0}".format(str(task_id))
        wait_until_in_statuses = (consts.TASK_STATUSES.running,
                                  consts.TASK_STATUSES.pending)
        timer = time.time()
        while True:
            call = self.run_cli_command(cmd_show_task)
            task = json.loads(call.stdout)
            if task['status'] not in wait_until_in_statuses:
