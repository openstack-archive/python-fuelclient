...
class TestNetworkTemplateCommand(test_engine.BaseCLITest):
    ...
    def test_network_template_upload(self):
        args = 'network-template upload --env 1 --dir /tmp/test-dir'
    ...
    def test_network_template_download(self):
        ...
        args = 'network-template download --env 1 --dir /tmp/test-dir'
        self.exec_command(args)

        self.m_get_client.assert_called_once_with('environment', mock.ANY)
        download_mock.assert_called_once_with(1, '/tmp/test-dir')

    def test_network_template_verify(self):
        verify_mock = self.m_client.verify_network_template
        verify_mock.return_value = {
            'status': 'ok',
            'message': 'message'
        }

        args = 'network-template verify --env 1'
        self.exec_command(args)
