...
class TestTaskCommand(test_engine.BaseCLITest):
    ...
    def test_task_list(self):
        ...
        self.m_client.get_all.assert_called_once_with(env_id=None,
                                                      statuses=None,
                                                      types=None)
    ...
    def test_task_list_w_parameters(self):
        ...
        types = ['provision', 'dump']
        args = 'task list -e {env_id} -m {statuses} -t {types}'.format(
            env_id=env_id,
        ...
        self.m_get_client.assert_called_once_with('task', mock.ANY)
        self.m_client.get_all.assert_called_once_with(env_id=env_id,
                                                      statuses=statuses,
                                                      types=types)
    ...
    def test_task_list_w_wrong_parameters(self, mocked_stderr):
        ...
        args = 'task list -m {statuses}'.format(statuses=' '.join(statuses))
        self.assertRaises(SystemExit, self.exec_command, args)
        self.assertIn('-m/--statuses: invalid choice',
