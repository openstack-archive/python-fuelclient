...
class TestTaskCommand(test_engine.BaseCLITest):
    ...
    def test_task_snapshot_config_download_json(self, m_dump):
        args = 'task snapshot get-default-config -f json -d /tmp'
        test_data = {'foo': 'bar'}
        ...
        m_open = mock.mock_open()
        with mock.patch('fuelclient.commands.task.open', m_open, create=True):
            self.exec_command(args)
    ...
    def test_task_snapshot_config_download_yaml(self, m_safe_dump):
        args = 'task snapshot get-default-config -f yaml -d /tmp'
        test_data = {'foo': 'bar'}
    ...
    def test_task_snapshot_create_w_config_json(self, _):
        args = 'task snapshot create -c /tmp/snapshot_conf.json'
        test_data = {'foo': 'bar'}
        expected_path = '/tmp/snapshot_conf.json'
    ...
    @mock.patch('fuelclient.utils.file_exists', return_value=True)
    def test_task_snapshot_create_w_config_yaml(self, _):
        args = 'task snapshot create -c /tmp/snapshot_conf.yaml'
        test_data = {'foo': 'bar'}
        expected_path = '/tmp/snapshot_conf.yaml'
