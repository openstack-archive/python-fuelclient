...
class TestTaskCommand(test_engine.BaseCLITest):
    ...
    def _test_cmd(self, cmd, method, cmd_line, client, expected_kwargs):
        self.m_get_client.reset_mock()
        self.m_client.get_filtered.reset_mock()
        self.m_client.__getattr__(method).return_value = 'data.yaml'
        self.exec_command('task {0} {1} {2}'.format(cmd, method, cmd_line))
    ...
    def test_task_deployment_info_download(self):
        self._test_cmd('deployment-info', 'download', '1',
                       'deployment-info',
                       dict(transaction_id=1, file_path=None))
