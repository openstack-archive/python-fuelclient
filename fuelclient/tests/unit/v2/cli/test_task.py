...
class TestTaskCommand(test_engine.BaseCLITest):
    ...
    def test_task_history_show(self):
        ...
        self.m_client.get_all_with_nodes_and_statuses.return_value = \
            utils.get_fake_deployment_history()
        self.exec_command(args)

        self.m_get_client.assert_called_once_with('deployment_history',
                                                  mock.ANY)
        self.m_client.get_all_with_nodes_and_statuses\
            .assert_called_once_with(transaction_id=task_id, nodes=None,
                                     statuses=None)
