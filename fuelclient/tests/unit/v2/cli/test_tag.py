...
class TestTagCommand(test_engine.BaseCLITest):
    ...
    def test_tag_download_json(self, m_dump):
        ...
        self.m_client.get_one.return_value = tag_data
        ...
        self.m_get_client.assert_called_once_with('tag', mock.ANY)
        self.m_client.get_one.assert_called_once_with(tag_id)
    ...
    def test_tag_update_json(self):
        tag_data = fake_tag.get_fake_tag()
        tag_id = tag_data['id']
        args = 'tag upload -t {} -f json -d /tmp'.format(tag_id)
        expected_path = '/tmp/tag_{}.json'.format(tag_id)
    ...
    def test_tag_update_yaml(self):
        tag_data = fake_tag.get_fake_tag()
        tag_id = tag_data['id']
        args = 'tag upload -t {} -f yaml -d /tmp'.format(tag_id)
        expected_path = '/tmp/tag_{}.yaml'.format(tag_id)
    ...
    def test_tag_delete(self):
        tag_id = 1
        args = 'tag delete -t {}'.format(tag_id)

        self.exec_command(args)
        self.m_get_client.assert_called_once_with('tag', mock.ANY)
        self.m_client.delete.assert_called_once_with(tag_id)
