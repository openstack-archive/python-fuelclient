...
class TestPluginsCommand(test_engine.BaseCLITest):
    ...
    def test_plugin_install(self):
        ...
        args = 'plugins install {0}'.format(path)
        self.exec_command(args)

        self.m_get_client.assert_called_once_with('plugins', mock.ANY)
        self.m_client.install.assert_called_once_with(path, force=False)

    def test_plugin_install_with_force(self):
        path = tempfile.mkstemp(suffix='.fp')[1]
        args = 'plugins install {0} --force'.format(path)
        self.exec_command(args)

        self.m_get_client.assert_called_once_with('plugins', mock.ANY)
        self.m_client.install.assert_called_once_with(path, force=True)
