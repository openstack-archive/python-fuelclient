...
class TestDeploymentGraphFacade(test_api.BaseLibTest):
    ...
    def test_graphs_download_release(self):
        matcher_get = self.m_request.get(
            '/api/v1/clusters/1/deployment_tasks/release/?graph_type=custom_graph',
            json=[]
        )
        self.client.download(env_id=1, level='release', graph_type='custom_graph')
        self.assertTrue(matcher_get.called)

    def test_graphs_download_plugins(self):
        matcher_get = self.m_request.get(
            '/api/v1/clusters/1/deployment_tasks/plugins/?graph_type=custom_graph',
            json=[]
        )
        self.client.download(env_id=1, level='plugins', graph_type='custom_graph')
        self.assertTrue(matcher_get.called)
    ...
    def test_graphs_download_cluster(self):
        ...
        self.client.download(env_id=1, level='cluster', graph_type='custom_graph')
        self.assertTrue(matcher_get.called)
