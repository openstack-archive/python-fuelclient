...
class TestTagFacade(test_api.BaseLibTest):
    ...
    def test_release_tag_list(self):
        owner, id = 'releases', 42
        expected_uri = self.get_object_uri(self.get_uri(owner),
                                           id,
                                           '/tags/')
        matcher = self.m_request.get(expected_uri, json=self.fake_tags)
        self.client.get_all('releases', id)

        self.assertTrue(matcher.called)

    def test_cluster_tag_list(self):
        owner, id = 'clusters', 42
        expected_uri = self.get_object_uri(self.get_uri(owner),
                                           id,
                                           '/tags/')
        matcher = self.m_request.get(expected_uri, json=self.fake_tags)
        self.client.get_all(owner_type='clusters', owner_id=id)

        self.assertTrue(matcher.called)

    def test_release_tag_download(self):
        owner, id = 'releases', 45
        expected_uri = self.get_object_uri(self.get_uri(owner),
                                           id,
                                           '/tags/{}/'.format(self.tag_name))
        role_matcher = self.m_request.get(expected_uri, json=self.fake_tag)

        role = self.client.get_one("releases", id, self.tag_name)

        self.assertTrue(expected_uri, role_matcher.called)
        self.assertEqual(role, self.fake_tag)

    def test_cluster_tag_download(self):
        owner, id = 'clusters', 45
        expected_uri = self.get_object_uri(self.get_uri(owner),
                                           id,
                                           '/tags/{}/'.format(self.tag_name))
        role_matcher = self.m_request.get(expected_uri, json=self.fake_tag)

        role = self.client.get_one("clusters", id, self.tag_name)
    ...
    def test_release_tag_delete(self):
        ...
        self.client.delete(owner, id, self.tag_name)
