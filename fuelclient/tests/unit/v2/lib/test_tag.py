...
class TestTagFacade(test_api.BaseLibTest):
    ...
    def get_uri(self, owner, owner_id, tag_name):
        return self.get_object_uri('/api/{version}/{owner}/'.format(
            version=self.version, owner=owner),
            owner_id,
            '/tags/{tag_name}/'.format(tag_name=tag_name))

    def test_release_tag_list(self):
        owner, owner_id = 'releases', 42
        expected_uri = self.get_uri(owner, owner_id, '')
        matcher = self.m_request.get(expected_uri, json=self.fake_tags)
        self.client.get_tag(owner, owner_id)
    ...
    def test_cluster_tag_list(self):
        ...
        expected_uri = self.get_uri(owner, owner_id, '')
        matcher = self.m_request.get(expected_uri, json=self.fake_tags)
        self.client.get_tag(owner_type=owner, owner_id=owner_id)

        self.assertTrue(matcher.called)

    def test_release_tag_download(self):
        owner, owner_id = 'releases', 45
        expected_uri = self.get_uri(owner, owner_id, self.tag_name)
    ...
    def test_release_tag_create(self):
        ...
        params = {"owner_type": owner,
                  "owner_id": owner_id}
        expected_uri = self.get_object_uri('/api/{version}/{owner}/'.format(
            version=self.version, owner=owner),
            owner_id,
            '/tags/')
