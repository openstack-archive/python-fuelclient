...
class TestReleaseDeploymentTasksActions(base.UnitTestCase):
    ...
    def test_release_tasks_download(self, mos):
        ...
        with patch('fuelclient.cli.serializers.open', new=m_open):
            self.execute(
                [
                    'fuel', 'rel', '--rel', '1',
                    '--deployment-tasks', '--download'
                ]
            )
        self.assertEqual(API_OUTPUT, m_open().getvalue())

    def test_release_tasks_upload(self, mos):
...
class TestSyncDeploymentTasks(base.UnitTestCase):
    ...
    def test_sync_deployment_scripts(self, mfiles):
    ...
    def test_sync_with_directory_path(self, mos, mfiles):
        self.m_request.get(rm.ANY, json=RELEASE_OUTPUT)
        put = self.m_request.put('/api/v1/releases/1/deployment_tasks',
                                 json={})
    ...
    def test_multiple_tasks_but_one_release(self, mfiles):
        self.m_request.get(rm.ANY, json=RELEASE_OUTPUT)
    ...
    def test_multiple_releases(self, mfiles):
        self.m_request.get(rm.ANY, json=MULTIPLE_RELEASES)
        put = self.m_request.put(rm.ANY, json={})
        mfiles.return_value = ['/etc/puppet/2014.2-6.0/tasks.yaml',
                               '/etc/puppet/2014.3-6.1/tasks.yaml']
        m_open = mock_open(read_data=API_OUTPUT)
        with patch('fuelclient.cli.serializers.open', new=m_open):
            self.execute(
                ['fuel', 'rel', '--sync-deployment-tasks']
            )
