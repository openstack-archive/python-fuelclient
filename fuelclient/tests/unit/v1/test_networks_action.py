...
class TestNetworkActions(base.UnitTestCase):
    ...
    def test_network_diff(self, mos, mopen, mrequests=None):
        ...
        mrequests.get('/api/v1/clusters/1/network_configuration/neutron',
                      json=input_data)
        with patch('fuelclient.cli.actions.network.DictDiffer') as mdictdiffer:
            read_data = {'key': 'value'}
            with patch('fuelclient.cli.actions.network.Environment.'
                       'read_network_data', return_value=read_data) as mread:
