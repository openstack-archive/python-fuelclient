...
class ExtensionClient(base_v1.BaseV1Client):
    ...
    def get_extensions(self, environment_id):
        ...
        return ext_obj.get_env_extensions()

    def enable_extensions(self, environment_id, extensions):
        ext_obj = self._entity_wrapper(environment_id)
        exts = ext_obj.get_env_extensions()
        enabled_exts = list(set(exts + extensions))
        return ext_obj.set_env_extensions(enabled_exts)

    def disable_extensions(self, environment_id, extensions):
        ext_obj = self._entity_wrapper(environment_id)
        exts = ext_obj.get_env_extensions()
        enabled_exts = list(set(exts) - set(extensions))
        return ext_obj.set_env_extensions(enabled_exts)
