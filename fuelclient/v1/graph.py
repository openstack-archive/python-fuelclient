...
class GraphClient(base_v1.BaseV1Client):
    ...
    def execute(self, env_id, nodes, graph_type=None, dry_run=False,
        ...
        def munge_subgraphs(subgraph):
            ...
            end_vertices_list = []
            if result:
                start_vertices = result.group(1)
                end_vertices = result.group(2)[1:]
                if start_vertices:
                    start_vertices_list = start_vertices.split(',')
                if end_vertices:
                    end_vertices_list = end_vertices.split(',')
            return {'start': start_vertices_list, 'end': end_vertices_list}
        if subgraphs:
            subgraphs_converted = list(six.moves.map(lambda s: munge_subgraphs(s),
                                                list(subgraphs)))
        url = "".join([
        ...
        deploy_data = self.connection.put_request(url, {'subgraphs': subgraphs_converted})
