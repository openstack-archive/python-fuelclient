...
class HealthClient(base_v1.BaseV1Client):
    ...
    _allowed_action_statuses = ("stopped", "restarted")
    ...
    def get_status_single(self, testset_id):
        data = self._entity_wrapper.get_tests_status(testset_id)
        if data:
            result = []
    ...
    def action(self, testrun_id, action_status):
        if action_status not in self._allowed_action_statuses:
            raise ValueError("action_status can be one of {}"
                             "".format(self._allowed_action_statuses))
