...
class NodeClient(base_v1.BaseV1Client):
    ...
    def get_all(self, environment_id=None, labels=None,
        ...
        node_collection = objects.NodeCollection(
            self._entity_wrapper.get_all())

        if environment_id:
            node_collection.filter_by_env_id(environment_id)
        if group_id:
            node_collection.filter_by_group_id(group_id)
        if roles:
            node_collection.filter_by_roles(roles)
        if status:
            node_collection.filter_by_status(status)
        if online != offline:
            node_collection.filter_by_online_status(online)

        if labels:
            result = [node for node in node_collection.data
                      if self._check_label(labels, node)]

        return node_collection.data
