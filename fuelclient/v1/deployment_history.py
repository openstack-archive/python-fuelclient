...
class DeploymentHistoryClient(base_v1.BaseV1Client):
    ...
    def get_all(self, transaction_id, nodes=None, statuses=None,
        ...
        if group_by_tasks:
            result = []
            for task_name, value in six.iteritems(tasks_parameters):
                statuses_by_node = []
                for record in history_records_by_task[task_name]:
                    record['time_start'] = record.get(
                        'time_start', u'').partition(u'.')[0]
                    record['time_end'] = record.get(
                        'time_end', u'not ended').partition(u'.')[0]
