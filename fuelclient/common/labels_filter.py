...
class LeftParenthesisOperator(Operator):
    ...
    @classmethod
    def evaluate(cls, operands_stack, ctx):
        raise ValueError("Right parenthesis is missing")
...
class EqualOperator(Operator):
    ...
    def evaluate(cls, operands_stack, ctx):
        ...
        if k not in ctx:
            ...
            operands_stack.append(ctx[k] == six.text_type(v))
...
class NotOperator(Operator):
    ...
    def evaluate(cls, operands_stack, ctx):
        if not len(operands_stack):
            raise ValueError("Invalid NOT operator position or operands")
...
class AndOperator(Operator):
    ...
    def evaluate(cls, operands_stack, ctx):
        if len(operands_stack) < 2:
            raise ValueError("Invalid AND operator position or operands")
        a = operands_stack.pop()
...
class OrOperator(Operator):
    ...
    def evaluate(cls, operands_stack, ctx):
        if len(operands_stack) < 2:
            raise ValueError("Invalid OR operator position or operands")
        a = operands_stack.pop()
        b = operands_stack.pop()
        operands_stack.append(
...
class RightParenthesisOperator(Operator):
    ...
    @classmethod
    def from_infix_to_rpn(cls, stack, out):
        while len(stack) and stack[-1] is not LeftParenthesisOperator:
            out.append(stack.pop())

        if not stack or stack[-1] is not LeftParenthesisOperator:
            raise ValueError("Left parenthesis is missing")
