...
class FuelClient(app.App):
    ...
    def run(self, argv):
        ...
        with exceptions_handler(self.options.debug):
            # Always run parent cliff app with `debug` option. We need
            # to catch exceptions which throws only when debug mode is
            # on.
            debug_option = '--debug'
            if debug_option not in argv:
                argv.append(debug_option)
