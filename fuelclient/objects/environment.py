...
class Environment(BaseObject):
    ...
    def get_vips_data_path(self, directory=os.curdir):
    ...
    def get_vips_data(self, ip_addr_id=None, network=None,
        ...
        if ip_addr_id:  # single vip is returned
    ...
    def write_vips_data(self, vips_data, directory=os.curdir, serializer=None):
        """Write VIP data to the given path.

        :param vips_data: vip data
        :type vips_data: list of dict
        :param directory: directory where to store downloaded config
        :type directory: str
        ...
        self._check_dir(directory)
        return (serializer or self.serializer).write_to_path(
            self.get_vips_data_path(directory),
            vips_data
        )
