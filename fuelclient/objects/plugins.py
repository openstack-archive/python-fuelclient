...
class BasePlugin(object):
    ...
    @classmethod
    def name_from_file(cls):
        raise NotImplementedError

    @classmethod
    def version_from_file(cls):
        raise NotImplementedError
...
class PluginV2(BasePlugin):
    ...
    @classmethod
    @master_only
    def install(cls, plugin_path):
        utils.exec_cmd('yum -y install {0}'.format(plugin_path))

    @classmethod
    @master_only
    def remove(cls, name, version):
        rpm_name = '{0}-{1}'.format(name, utils.major_version(version))
    ...
    def name_from_file(cls, file_path):
        ...
        return cls._remove_major_version(name)
    ...
    @classmethod
    def _remove_major_version(cls, name):
...
class Plugins(base.BaseObject):
    ...
    def register(cls, name, version, force=False):
        ...
        """
        metadata = None
        for m in utils.walk_and_parse_yaml(METADATA_MASK):
        ...
        cls.update_or_create(metadata, force=force)
    ...
    def sync(cls):
        ...
        in API service.
        """
        for metadata in utils.walk_and_parse_yaml(METADATA_MASK):
    ...
    @classmethod
    def install(cls, plugin_path, force):
        ...
        cls.register(name, version, force=force)
    ...
    def make_obj_by_name(cls, name, version):
        ...
        :raises: error.BadDataException unsupported plugin format
        """
        plugin = cls.get_plugin(name, version)
    ...
    def update_or_create(cls, metadata, force=False):
        ...
        if plugin_for_update:
            ...
            cls.connection.put_request(url, metadata)

            return

        # If plugin is not updatable it means that we should
        # create new instance in Nailgun
        resp = cls.connection.post_request_raw(cls.class_api_path, metadata)
    ...
    def get_plugin_for_update(cls, metadata):
        ...
        plugins = filter(
            ...
            utils.major_version(metadata['version']) == \
            utils.major_version(p['version']),
