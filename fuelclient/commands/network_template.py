...
class NetworkTemplateUpload(NetworkTemplateMixin, base.BaseCommand):
    ...
    def take_action(self, parsed_args):
        self.client.upload_network_template(parsed_args.env, parsed_args.dir)
...
class NetworkTemplateDownload(NetworkTemplateMixin, base.BaseCommand):
    ...
    def take_action(self, parsed_args):
        network_file_path = self.client.download_network_template(
            parsed_args.env, parsed_args.dir)

        msg = ("Network configuration for environment with id={0}"
               " downloaded to {1}\n").format(
                   parsed_args.env, network_file_path)
        self.app.stdout.write(msg)
...
class NetworkTemplateVerify(NetworkTemplateMixin, base.BaseCommand):
    """To verify network configuration for specified environment:
            fuel2 network-template verify --env 1
    """

    def get_parser(self, prog_name):
        parser = super(NetworkTemplateVerify, self).get_parser(prog_name)
    ...
    def take_action(self, parsed_args):
        response = self.client.verify_network_template(parsed_args.env)
