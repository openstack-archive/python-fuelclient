...
class BaseListCommand(lister.Lister, BaseCommand):
    ...
    def get_parser(self, prog_name):
        ...
        group.add_argument('-s',
                           ...
                           default='id',
                           help='The key for sorting the data. Defaults to id')

        return parser

    def take_action(self, parsed_args):
        client = fuelclient.get_client(self.entity_name, VERSION)

        data = client.get_all()
        data = data_utils.get_display_data_multi(self.columns, data)

        scolumn_id = self.columns.index(parsed_args.sort_column)
        data.sort(key=lambda x: x[scolumn_id])
...
class BaseShowCommand(show.ShowOne, BaseCommand):
    ...
    def take_action(self, parsed_args):
        ...
        data = client.get_by_id(parsed_args.id)
...
class BaseDeleteCommand(BaseCommand):
    ...
    def take_action(self, parsed_args):
        ...
        client.delete_by_id(parsed_args.id)
