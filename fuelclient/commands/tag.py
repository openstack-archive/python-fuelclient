...
class BaseTagAssignee(TagMixIn, base.BaseCommand):
    ...
    def take_action(self, parsed_args):
        self.client.assignee(node=parsed_args.node,
                             tag_ids=parsed_args.tags)

        self.app.stdout.write(self.message.format(t=parsed_args.tags,
                                                  n=parsed_args.node))
...
class TagAssign(BaseTagAssignee):
    ...
    action = 'assign'
    message = 'Tags {t} were assigned to the node {n}.\n'

    @property
    def assignee(self):
        return self.client.assign(kwargs)
...
class TagUnassign(BaseTagAssignee):
    ...
    action = 'unassign'
    message = 'Tags {t} were unassigned from the node {n}.\n'
