...
class TagUpdate(TagMixIn, base.BaseShowCommand):
    """Update a tag from file description."""
    columns = TagShow.columns

    def get_parser(self, prog_name):
        # to avoid conflict with '-f' aka formatter option
        parser = command.Command.get_parser(self, prog_name)
        parser.add_argument(
            '-t',
            '--tag_id',
            type=int,
        ...
        formatter_group.add_argument(
            '--formatter',
            required=False,
            choices=sorted(self._formatter_plugins.names()),
            default=self.formatter_default,
            help='Format of serialized tag description.')
        formatter_group.add_argument(
            '-c', '--column',
            action='append',
    ...
    def take_action(self, parsed_args):
        ...
        return self.client.update(parsed_args.tag_id, data), self.columns
...
class TagCreate(TagMixIn, base.BaseCommand):
    ...
    def take_action(self, parsed_args):
        ...
        model, model_id = self.parse_model(parsed_args)
        self.client.create(model, model_id, data)
