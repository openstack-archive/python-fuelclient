...
def exceptions_handler(debug=False):
    ...
    # and print meaningful reason
        ...
        exit_with_error(
            textwrap.dedent("""Can't connect to Nailgun server!
        ...
        error = """
        Error: Unexpected error happened while attempting to execute operation.
        Error: {0}.

        Please file a bug report at https://bugs.launchpad.net/fuel/.

        Execute again the last command with --debug option. Please include the output in your bug report."""

        exit_with_error(textwrap.dedent(error.format(six.text_type(e))))
