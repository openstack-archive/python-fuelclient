...
class NetworkTemplateAction(Action):
    ...
    def __init__(self):
        ...
        self.args = (
            ...
            group(
                ...
                Args.get_verify_arg(
                    "Verify current network template configuration."),
                Args.get_upload_arg(
    ...
    def upload(self, params):
        ...
        env.set_network_data(network_data)
        print("Network template {0} has been uploaded.".format(params.dir))

    def verify(self, params):
        """Verifies specified network template for environment:
                fuel --env 1 network --verify --dir path/to/directory
        """
        print("Network template verification is not implemented yet")
    ...
    def download(self, params):
        ...
        """
        env = Environment(params.env)
        network_data = env.get_network_template_data()
        network_file_path = env.write_network_template_data(
            network_data,
            directory=params.dir,
            serializer=self.serializer)

        print("Network template configuration for environment with id={0}"
              " downloaded to {1}".format(env.id, network_file_path))
