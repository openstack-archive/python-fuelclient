...
class PluginAction(Action):
    ...
    def __init__(self):
        ...
        self.args = [
            ...
            Args.get_plugin_register_arg("Register action"),
            Args.get_plugin_unregister_arg("Unregister action"),
            Args.get_plugin_update_arg("Update action"),
            Args.get_plugin_downgrade_arg("Downgrade action"),
            Args.get_plugin_sync_arg("Sync action"),
    ...
    def install(self, params):
        """Install plugin archive and register in API service
               ...
               fuel plugins --install plugin-name.fb
               fuel plugins --install plugin-name-2.0-2.0.1-0.noarch.rpm
        """
        results = Plugins.install(params.install, params.force)
        self.serializer.print_to_output(
            results,
    ...
    def unregister(self, params):
        ...
        Plugins.unregister(name, version)
    ...
    def parse_name_version(self, param):
        ...
        if 2 < len(attrs) or len(attrs) < 2:
            raise error.ArgumentException(
                'Syntax: fuel plugins <action> fuel_plugin==1.0')
